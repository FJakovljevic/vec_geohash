...
class TestTile(unittest.TestCase):
    def test_tile_single(self):
        ...
        np.testing.assert_array_equal(vec_geohash.tile_to_quadkey(tile_x, tile_y, zoom), expected_output)

        # expected output for latlon
        expected_output = np.array([[0., -85.05112878, 180., 0.]])
        np.testing.assert_almost_equal(vec_geohash.tile_to_lat_lon(tile_x, tile_y, zoom), expected_output)

        # expected output for pixel
        expected_output = np.array([256, 256, 512, 512])
        np.testing.assert_array_equal(vec_geohash.tile_to_pixel(tile_x, tile_y), expected_output)
    ...
    def test_tile_vec(self):
        ...
        # expected output for latlon
        expected_output = np.array([[-180., 66.5132604, -90., 85.0511288], [90., -85.0511288, 180., -66.5132604]])
        np.testing.assert_almost_equal(vec_geohash.tile_to_lat_lon(tile_x, tile_y, zoom), expected_output)
...
class TestQuadkey(unittest.TestCase):
    def test_quadkey_single(self):
        ...
        np.testing.assert_almost_equal(vec_geohash.quadkey_to_lat_lon(quadkey), expected_output)

        # expected output for pixel
        expected_output = np.array([[95488, 42496, 95744, 42752]])
        np.testing.assert_almost_equal(vec_geohash.quadkey_to_pixel(quadkey), expected_output)
    ...
    def test_quadkey_vector(self):
        ...
        # expected output for pixel
        expected_output = np.array([[95488, 42496, 95744, 42752], [33536, 48640, 33792, 48896]])
        np.testing.assert_almost_equal(vec_geohash.quadkey_to_pixel(quadkey), expected_output, decimal=3)
...
class TestProjection(unittest.TestCase):
    ...
    def test_project(self):
        ...
        expected_longitudes = np.array([0.5, 0.625, 0.75, 0.875, 1, 0.375, 0.25, 0.125, 0])

        latitudes = np.array([0, 30, 60, -30, -60])
        expected_latitudes = np.array([0.5, 0.4125752, 0.2903996, 0.5874248, 0.7096004])

        projected_longitudes, projected_latitudes = vec_geohash.project(latitudes, longitudes)
